...
class TerraformStackConfigurationTests(unittest.TestCase):
    ...
    def test_opc_ownership_and_codex_npm_prefix_are_enforced(self) -> None:
        ...
        self.assertIn('npm install -g --include=optional "${package}" "${platform_package}"', SCRIPTS["install_codex.sh"])
        self.assertIn("@openai/codex-linux-x64", SCRIPTS["install_codex.sh"])
